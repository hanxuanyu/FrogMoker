...
- **XML 格式**: 使用 `${变量名}` 作为占位符
- **MAP 格式**: 使用 `{{变量名}}` 作为占位符，适用于 HTTP 表单请求
...
支持通过不同协议发送报文，具有丰富的参数配置能力：

- **HTTP 客户端**: 支持 GET/POST/PUT/DELETE 等方法
- **参数类型**: 支持文本、数字、布尔、下拉选择、键值对映射、数组等多种类型
- **参数联动**: 支持根据其他参数值动态显示/隐藏参数
- **扩展性**: 可通过实现 `ProtocolClient` 接口添加新协议

**HTTP 客户端特性：**
- 自定义请求头（MAP 类型）
- 查询参数配置（MAP 类型）
- 表单数据配置（MAP 类型，支持从模板快速填充）
- Content-Type 智能联动（仅在有 body 的请求方法时显示）
- 超时时间配置
- 重定向控制
